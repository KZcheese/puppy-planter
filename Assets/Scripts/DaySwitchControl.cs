...
public class DaySwitchControl : MonoBehaviour {
    ...
    public void GoNextDayButton() {
        ...
        SavingText.text += "Sales                        " +
                           (GameManager.Instance.Money - GameManager.Instance.YesterdayMoney +
                            GameManager.Instance.TodayUpdateCost).ToString("c2") + "\n";
    ...
    private void CostCalculate() {
        var dogCost = GameManager.Instance.DogList.Count * GameManager.Instance.CostPerDog;
        ...
        if (GameManager.Instance.DayCount % 6 == 0) {
            ...
            foreach (var dog in GameManager.Instance.DogList) dog.HP -= 1;
        }
    ...
    public void GoContinueButton() {
        ...
        if (MoneyCal.activeSelf) {
            ...
            if (GameManager.Instance.Money < 0) {
                ...
                var _dayAll = GameManager.Instance.DayCount;

                var _weekDay = _dayAll % 5;
                switch (_weekDay) {
                    case 1:
    ...
    private void ReminderPopUp(int Day) {
